...
# ==========================================================
# 1. PREP: Stats for "Mean ± SD"
# ==========================================================
# We calculate the summary stats first to use in the labels
summary_stats <- forest_long %>%
  ...
  mutate(
    # \u00B1 is the unicode for the "plus-minus" sign
    label_text = paste0(round(mean_val, 1), " \u00B1 ", round(sd_val, 1))
  )
...
p <- ggplot(forest_long, aes(x = Year_Category, y = Forest_Loss, fill = Year_Category)) +
  ...
  # C. Mean Marker (White diamond)
  stat_summary(fun = mean, geom = "point", shape = 23, size = 3, fill = "white", color = "black") +
  
  # D. Annotation: "Mean ± SD" at y = 17
  geom_text(data = summary_stats, 
            aes(x = Year_Category, y = 17, label = label_text), 
            inherit.aes = FALSE, 
            size = 3.5,          # Adjust size for journal requirements
            color = "black") +
  ...
  # F. THEME SETTINGS
  # ---------------------------------------------------------
theme_classic() +  # Provides white background and black axis lines
  theme(
    legend.position = "none",          # Remove legend if redundant (x-axis explains it)
    axis.text = element_text(color = "black", size = 10), # Ensure text is black, not grey
    axis.title = element_text(color = "black", size = 11, face = "bold"),
    panel.grid.major = element_blank(), # Remove gridlines
    panel.grid.minor = element_blank(),
    axis.line = element_line(color = "black", linewidth = 0.5) # Explicit black axis lines
  ) +
  # ---------------------------------------------------------
...
# Standard: Use Neutral for Baseline (Grey), Bold for Event (Vermilion)
# This ensures distinctness in both color and grayscale printing.
springer_palette <- c("#F76D5E", "#D55E00")
